...
var hexRadius = d3.min([width/((MapColumns + 0.5) * Math.sqrt(3)),
    height/((MapRows + 1/3) * 1.5)]);//找出一个数组中的最小值 六边形的最大半径
...
for (var i = 0; i < MapRows; i++) {
    for (var j = 0; j < MapColumns; j++) {
        points.push([hexRadius * j * 1.75, hexRadius * i * 1.5]);//每个六边形的中心位置
    }//for j
}//for i
...
var dataset=[];
var dataset2=[];
d3.json("model.json",function(json){
    dataset=json.data;
    dataset2=json.matrix;
    console.log(arrayColor);
    for(var i=0;i<dataset.length;i++)
    {
        for(var j=0;j<dataset[i].length;j++)
        {
            array.push(dataset[i][j]);
         }
    }
...
//Create SVG element
var svg = d3.select("#chart").append("svg") //chart是div的id
...
svg.append("g")
    ...
    .style("fill", function (d,i) {
        return colorRange(arrayColor[i]);
    });
...
svg.append("g")
    ...
    .attr("class", "hexagon")
    .attr("d", function (d,i) {
        return "M" + d.x + "," + d.y + hexbin.hexagon(radius(array[i].length));
    })
    .attr("stroke", function (d,i) {
        return "#fff";
    })
